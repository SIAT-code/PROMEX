...
class AbstractModel(pl.LightningModule):
    def __init__(self,
        ...
        if from_checkpoint:
            self.load_checkpoint(from_checkpoint, load_prev_scheduler, stage='train')  # add item: stage mark
    ...
    @staticmethod
    def load_weights(model, weights, stage):  # add: item: stage mark
        ...
        for k, v in weights.items():
            ...
            if k in model_dict.keys() and ('classifier' not in k if stage == 'train' else 1):  # add 'classifier' not in k if stage=='train' mark
                model_dict[k] = v
    ...
    def load_checkpoint(self, from_checkpoint, load_prev_scheduler, stage):  # add: item: stage mark
        state_dict = torch.load(from_checkpoint, map_location=self.device)
        self.load_weights(self.model, state_dict["model"], stage)  # add: item: stage mark
