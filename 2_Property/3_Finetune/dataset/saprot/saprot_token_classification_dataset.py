...
class SaprotTokenClassificationDataset(LMDBDataset):
    ...
    def __getitem__(self, index):
        ...
        #----------------------------- mark -----------------------------#
        label = list(map(int, label))[:self.max_length]
        #----------------------------------------------------------------#
    ...
    def collate_fn(self, batch):
        ...
        #---------------------------- mark -----------------------------------#
        max_seq_length = max(map(len, [seq.split() for seq in seqs]))
        label_ids = [label + [-100] * (max_seq_length - len(label)) for label in label_ids] 
        #---------------------------------------------------------------------#
