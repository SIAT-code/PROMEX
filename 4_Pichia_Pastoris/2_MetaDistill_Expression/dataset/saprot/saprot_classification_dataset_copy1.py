...
class SaprotClassificationDataset(LMDBDataset):
    ...
    def __getitem__(self, index):
        if self.indices and self.length:
            entry = json.loads(self._get(self.indices[index]))
        else:
    ...
    def __len__(self):
        return self.length if self.indices and self.length else int(self._get("length"))
    ...
    def _meta_dataloader(self, stage):
        ...
        length = int(dataset._get("length"))
        indices = list(range(length))
        random.shuffle(indices)
        split_idx = length // 2
        adapt_indices = indices[:split_idx]
        eval_indices = indices[split_idx:]
        
        adapt_dataset = copy.copy(dataset)
        eval_dataset = copy.copy(dataset)
        
        meta_dataset = MetaSaprotClassificationDataset(adapt_dataset, eval_dataset, adapt_indices, eval_indices, self.dataloader_kwargs)
...
class MetaSaprotClassificationDataset(Dataset):
    def __init__(self,
        ...
        self.support_iters = []
        self.query_iters = []
        for _ in range(dataloader_kwargs['iters']):
        
            adapt_dataset.indices = adapt_indices
            adapt_dataset.length = dataloader_kwargs['adapt_batch_size'] * dataloader_kwargs['adapt_steps']
            eval_dataset.indices = eval_indices
            eval_dataset.length = dataloader_kwargs['eval_batch_size']
            
            support_iter = DataLoader(adapt_dataset, collate_fn=adapt_dataset.collate_fn, batch_size=dataloader_kwargs['adapt_batch_size'], num_workers=dataloader_kwargs['num_workers'])
            query_iter = DataLoader(eval_dataset, collate_fn=eval_dataset.collate_fn, batch_size=dataloader_kwargs['eval_batch_size'], num_workers=dataloader_kwargs['num_workers'])
