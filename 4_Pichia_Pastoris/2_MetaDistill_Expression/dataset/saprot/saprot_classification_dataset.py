...
class SaprotClassificationDataset(LMDBDataset):
    ...
    def __getitem__(self, index):
        ...
        name = entry["name"]  # add mark
        return name, seq, label, coords  # origin: (return seq, label, coords)  mark

    def __len__(self):
        return self.length if self.indices is not None and self.length is not None else int(self._get("length"))

    def collate_fn(self, batch):
        names, seqs, label_ids, coords = tuple(zip(*batch))  # origin: (seqs, label_ids, coords = tuple(zip(*batch)))  # mark

        label_ids = torch.tensor(label_ids, dtype=torch.long)
        labels = {"names": names, "labels": label_ids}  # origin: labels = {"labels": label_ids}  mark
