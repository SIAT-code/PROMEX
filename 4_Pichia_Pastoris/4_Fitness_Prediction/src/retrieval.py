...
class Retriever():
    ...
    def __call__(self, sequences=None):  # mark modify a lot
        args = self.args
        if args.mode == 'vectorize':
            for key, proteins in sequences.items():
                ...
                save_path = save_path.replace('vectors', f'{key}_vectors')
                make_dir(save_path)
                torch.save(vectors, save_path)
        else:
            save_path = self.get_save_path('vectorize')
            aux_names, aux_vectors = torch.load(save_path.replace('vectors', f'auxiliary_vectors'), weights_only=False).values()
            tgt_names, tgt_vectors = torch.load(save_path.replace('vectors', f'target_vectors'), weights_only=False).values()
            
            
            if args.top_k >= 1:
                topk, topk_idx = self.compute_nns(tgt_vectors, aux_vectors, args.top_k)
            else:
                topk, topk_idx = self.compute_nns(tgt_vectors, aux_vectors, aux_vectors.shape[0])
                
            results = {name: dict(tgt_names=[aux_names[i] for i in indices], scores=scores) \
                           for name, scores, indices in zip(tgt_names, topk, topk_idx)}
