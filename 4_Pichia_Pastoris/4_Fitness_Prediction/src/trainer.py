...
class TrainerBase(ABC):
    ...
    @abstractmethod
    def compute_metrics(self, predicts, targets):
        ''' Return a dict of metrics'''
        pass
    ...
    def evaluate_epoch(self, eval_iter):
        ...
        with torch.no_grad():
            for batch in pbar:
                ...
                # labels.append(batch['labels'].to('cpu'))  # mark origin
        
        # predicts, targets, labels = torch.cat(predicts), torch.cat(targets), torch.cat(labels)  # mark origin
        predicts, targets = torch.cat(predicts), torch.cat(targets)
        # logs = self.compute_metrics(predicts, targets, labels)  # mark origin
        logs = self.compute_metrics(predicts, targets)
...
class MetaRankingTrainer(RankingTrainer):
    ...
    def compute_metrics(self, predicts, targets):
        logs = defaultdict(float)
        for batch_preds, batch_tgts in zip(predicts, targets):
            log = super().compute_metrics(batch_preds, batch_tgts)
            for key, value in log.items():
                logs[key] += value
    ...
    def evaluate_epoch(self, eval_iter):
        ...
        for batch in pbar:
            ...
            # batch_labels = [eval_batch['labels'].to('cpu') for eval_batch in batch['eval_batches']]  # mark origin
            
            # log = self.compute_metrics(batch_preds, batch_tgts, batch_labels)  # mark origin
            log = self.compute_metrics(batch_preds, batch_tgts)
