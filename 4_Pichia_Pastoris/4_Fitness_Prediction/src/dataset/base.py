...
class MutantSequenceData(Dataset):
    ...
    def __getitem__(self, idx):
        # return self.wt_aas[idx], self.mt_aas[idx], self.positions[idx], self.DMS_score[idx], self.DMS_score_bin[idx]  # mark
        return self.wt_aas[idx], self.mt_aas[idx], self.positions[idx], self.DMS_score[idx]
    def collate(self, raw_batch):
        # wt_aas, mt_aas, positions, scores, labels = zip(*raw_batch)  # mark origin
        wt_aas, mt_aas, positions, scores = zip(*raw_batch)
