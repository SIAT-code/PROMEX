...
# metrics = ['spearmanr', 'ndcg', 'topk_pr']  # mark origin
metrics = ['spearmanr', 'ndcg']
...
def compute_scores(predicts, targets, k=30):
...
def group_scores(train_df, pred_df, test_df, k=30):
    ...
    for name in group_names:
        ...
        report.append(compute_scores(pred_df.loc[indices].to_list(),
                                    ...
                                    #  test_df.loc[indices, 'DMS_score_bin'].to_list(),  # mark origin
                                     k))
    report.append(compute_scores(pred_df.loc[test_df.index].to_list(),
                                 test_df['DMS_score'].to_list(),
                                #  test_df['DMS_score_bin'].to_list(),    # mark origin
                                 k))
