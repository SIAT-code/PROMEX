...
from .utils.data import make_dir, split_data, split_train_test_data  # mark
...
class Pipeline():
    ...
    def get_base_model(self, load_dir=None):
        args = self.args
        esm2_model_name = config.model_dir["esm2"]
        promex_model_name = config.model_dir["promex"]
        if load_dir is None:
            model = EsmForMaskedLM.from_pretrained(esm2_model_name)
            if os.path.isfile(promex_model_name):
                promex_checkpoint = torch.load(promex_model_name, map_location="cpu", weights_only=False)
                promex_model = promex_checkpoint["model"] if isinstance(promex_checkpoint, dict) and "model" in promex_checkpoint else promex_checkpoint
                promex_model = {k: v for k, v in promex_model.items() if not k.startswith("classifier.")}
                model.load_state_dict(promex_model, strict=False)
    ...
    def finetune_single(self, train, valid, save_dir=None, tgt_name=None):
        args = self.args
        if 'transfer' in args.mode:
            load_dir = self.get_save_dir('finetune' if args.mode == 'transfer' else 'meta', tgt_name)  # mark origin: train['name']
            logs = torch.load(load_dir + '/logs.pkl', weights_only=False)
            if args.lora_r == 0:
    ...
    def finetune_single_cv(self, train, test=None, tgt_name=None):
        args = self.args
        save_dir = self.get_save_dir(args.mode, tgt_name)  # mark train['name']
        if args.cross_validation <= 1:
            report = self.finetune_single(train=train, valid=test, save_dir=save_dir, tgt_name=tgt_name)
        ...
        for i, (cv_train, cv_valid) in enumerate(splits):
            ...
            cv_report = self.finetune_single(train=cv_train, valid=cv_valid, save_dir=None, tgt_name=tgt_name)
        ...
        logs = self.finetune_single(train=train, valid=None, save_dir=save_dir, tgt_name=tgt_name)
    ...
    def meta_single(self, train, eval_train, eval_test=None):
        ...
        # save_dir = self.get_save_dir(args.mode, eval_train['name'])  # mark origin
        save_dir = self.get_save_dir(args.mode, eval_test['name'])
    ...
    def test_single(self, train, test, compute_test_score=False):
        args = self.args
        if args.epochs > 0:
        ...
        # report, _ = group_scores(train['df'], predicts, test['df'])  # mark origin
        report, _ = group_scores(train['df'], predicts, test['df']) if compute_test_score else (None, None)
    ...
    def get_meta_database(self, all_proteins):
        ...
        database = {
                ...
                if (best_dataset := max(datasets, key=lambda x: len(x['df']))) and len(best_dataset['df']) >= 2 * args.list_size
        }
        # database = {name: max(datasets, key=lambda x: len(x['df'])) \
        #                 for name, datasets in all_proteins.items()}  # mark origin
        topk = torch.load(f'{config.retr_dir}/topk_esm2_{args.retr_metric}.pkl', weights_only=False)
    ...
    def __call__(self, aux_proteins, tgt_proteins):
        args = self.args
        proteins = self.select_datasets(tgt_proteins)
        if args.mode == 'meta' or args.mode == 'meta-transfer':  # mark origin: if args.mode == 'meta'
            database, topk = self.get_meta_database(aux_proteins)


        running_proteins = ['S7_xylanase', 'Pel114_pectate_lyase']
        reports = {}
        for protein in proteins:
            #------------------------- mark ---------------------------#
            if protein['name'] not in running_proteins:
                continue
            #----------------------------------------------------------#
            print(f'**********************Current dataset: {protein["name"]}**********************')

            tgt_name = protein['name']
            all_src_names = topk[tgt_name]['tgt_names']
            src_names = [name for name in all_src_names if name in database]
            train = database[src_names[0]]
            test = protein.copy()
            
            if args.test:
                report = self.test_single(train, test, args.compute_test_score)
            elif args.mode != 'meta':
                if args.mode == 'finetune' and args.augment:
                    protein = self.augment_data(protein)[0]
                report = self.finetune_single_cv(train, test, tgt_name=test['name'])  # mark
            else:
                meta_train = [database[name] for name in src_names[1: args.meta_tasks + 1]]  # mark skip the first auxiliary task which is as train set.
                if args.augment:
        ...
        # if args.test:
        # if args.test and args.protein in {'single-site', 'multi-site', 'all'}:  # mark origin
        if args.test and args.protein in {'single-site', 'multi-site', 'all'} and args.compute_test_score:
