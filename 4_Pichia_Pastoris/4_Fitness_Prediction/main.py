...
from src.dataset.saprot import SaProtMutantData, saprot_zero_shot
from src.pipeline import Pipeline
from src.utils.score import metrics
...
def parse_args():
    ...
    #------------------------------------------------- mark add --------------------------------------------------------------#
    parser.add_argument('--compute_test_score', '-cts', action='store_true',
                        help='Whether it is need to compute test score. It is False while test set labels are not known.')
    #-------------------------------------------------------------------------------------------------------------------------#
...
if __name__ == '__main__':
    ...
    if args.mode == 'transfer':
        assert args.augment
    auxiliary_path = config.auxiliary_data_path.replace('.pkl', f'{args.save_postfix}.pkl')
    target_path = config.target_data_path.replace('.pkl', f'{args.save_postfix}.pkl')
    aux_proteins = torch.load(auxiliary_path, weights_only=False)
    tgt_proteins = torch.load(target_path, weights_only=False)
    ...
    pipeline(aux_proteins, tgt_proteins)
