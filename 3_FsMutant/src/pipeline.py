...
class Pipeline():
    ...
    def finetune_single(self, train, valid, save_dir=None, split_n=None):
        args = self.args
        if 'transfer' in args.mode:
            load_dir = self.get_save_dir('finetune' if args.mode == 'transfer' else 'meta', train['name'])
            load_dir = f'{load_dir}_split[{split_n}]'  # mark
    ...
    def finetune_single_cv(self, train, test=None, split_n=None):
        args = self.args
        save_dir = self.get_save_dir(args.mode, train['name'])
        save_dir = f'{save_dir}_split[{split_n}]'  # mark
        if args.cross_validation <= 1:
            report = self.finetune_single(train, test, save_dir, split_n=split_n)
        ...
        for i, (cv_train, cv_valid) in enumerate(splits):
            ...
            cv_report = self.finetune_single(cv_train, cv_valid, split_n=split_n)
        ...
        logs = self.finetune_single(train, None, save_dir, split_n=split_n)
        report['train_loss'] = logs['train_loss']
        torch.save(report, save_dir + '/logs.pkl')
        args.epochs = epochs
        return report
    
    def meta_single(self, train, eval_train, eval_test=None, split_n=None):
        args = self.args
        model, tokenizer = self.get_base_model()
        if args.lora_r > 0:
            lora_config = LoraConfig(r=args.lora_r,
        ...
        trainer = MetaRankingTrainer(model.to(self.device),
                                     ...
                                     save_dir=save_dir,
                                     patience=args.patience,
                                    teacher_name=args.teacher_name,  # mark add parameter
                                    teacher_checkpoint=args.teacher_checkpoint,  # mark add parameter
                )
    ...
    def test_single(self, train, test, test_name=None, split_n=None):
        args = self.args
        if args.epochs > 0:
            load_dir = self.get_save_dir(args.mode, test['name'])
            load_dir = f'{load_dir}_split[{split_n}]'
            if args.lora_r == 0:
                model, tokenizer = self.get_base_model(load_dir)
            else:
                model, tokenizer = self.get_base_model()
        ...
        save_path += f'/base_{test_name}_split[{split_n}].csv' if args.epochs == 0 else f'/{test_name}_split[{split_n}].csv'
        make_dir(save_path)
    ...
    def __call__(self, all_proteins):
        ...
        for protein in proteins:
            ...
            train_test_dict = split_train_test_data(protein, args.train_size, config)
            
            report_splits = {}
            for split_n in range(1, 6):
                if f'{args.train_size}_{split_n}' not in train_test_dict:
                    continue
                train = train_test_dict[f'{args.train_size}_{split_n}']['train']
                compound_multi = train_test_dict[f'{args.train_size}_{split_n}']['compound_multi']
                local_single = train_test_dict[f'{args.train_size}_{split_n}']['local_single']
                cross_single = train_test_dict[f'{args.train_size}_{split_n}']['cross_single']
                
                if args.test:
                    report_compound_multi = self.test_single(train, compound_multi, 'compound_multi_split', split_n=split_n) \
                                            if compound_multi['df'] is not None else None
                    report_local_single = self.test_single(train, local_single, 'local_single_split', split_n=split_n) \
                                            if local_single['df'] is not None else None
                    report_cross_single = self.test_single(train, cross_single, 'cross_single_split', split_n=split_n) \
                                            if cross_single['df'] is not None else None
                elif args.mode != 'meta':
                    if args.mode == 'finetune' and args.augment:
                        protein = self.augment_data(protein)[0]
                    report_compound_multi = self.finetune_single_cv(train, compound_multi, split_n=split_n) \
                                            if compound_multi['df'] is not None else None
                    report_local_single = self.finetune_single_cv(train, local_single, split_n=split_n) \
                                            if local_single['df'] is not None else None
                    report_cross_single = self.finetune_single_cv(train, cross_single, split_n=split_n) \
                                            if cross_single['df'] is not None else None
                else:
                    ...
                    report_compound_multi = self.meta_single(meta_train, train, compound_multi, split_n=split_n) \
                                                if compound_multi['df'] is not None else None
                    report_local_single = self.meta_single(meta_train, train, local_single, split_n=split_n) \
                                                if local_single['df'] is not None else None
                    report_cross_single = self.meta_single(meta_train, train, cross_single, split_n=split_n) \
                                                if cross_single['df'] is not None else None
                    
                report_splits[f'{args.train_size}_{split_n}'] = {'compound_multi': report_compound_multi, 'local_single': report_local_single, 'cross_single': report_cross_single}
                
            reports[protein['name']] = report_splits
