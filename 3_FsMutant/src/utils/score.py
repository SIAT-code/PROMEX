...
def compute_scores(predicts, targets, labels, k=30):
    ...
    report['ndcg'] = ndcg_score(std_tgts, [predicts], k=10) 
...
def summarize_scores(score_groups, save_path=None):  # mark
            ...
            [
                {
                    ...
                    **{'splits': split_name},
                    **{
                        f'[{test_name}]size': tuple(group['size'].tolist())
                        for test_name, group in groups.items() if group is not None
                    },
                    **{
                        f'[{test_name}]{group_name}': row[metric]
                        for test_name, group in groups.items() if group is not None
                        for group_name, row in group.iterrows()
                    }
                }
                for split_name, groups in score_groups[protein_name].items()
            ]
            for protein_name in score_groups.keys()
        ]
        reports = sum(reports, [])
        reports = pd.DataFrame(reports)
        summary[metric] = reports
