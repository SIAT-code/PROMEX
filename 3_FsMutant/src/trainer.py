...
class MetaRankingTrainer(RankingTrainer):
    def __init__(self, model, 
        ...
        **kwargs):
        super().__init__(model, **kwargs)
        self.teacher = TeacherModel(num_labels=1, dim=1280, depth=1, heads=20, mlp_dim=2560, dropout=0.1, pool='mean').to(model.device)
        self.load_teacher_checkpoint(self.teacher, teacher_checkpoint)
        self.teacher.eval()
    ...
    def fast_adapt(self, adapt_batch, eval_batch, training=True):
        ...
        for batch in adapt_batch:
            s__hidden_state = self.model(**batch['sequences'], output_hidden_states=True).hidden_states[-1]
            with torch.no_grad():
                t_logits, t_hidden_state = self.teacher(batch['sequences'])
            alpha = 0.5
            adapt_loss = (1 - alpha) * self.compute_loss(batch) + alpha * self.compute_l2_loss(s__hidden_state, t_hidden_state)  # mark origin alpha=0.5
            adapt(adapt_loss)
